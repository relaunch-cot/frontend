...
async function enviarNotificacaoMensagem(receiverId, messageContent) {
  try {
    // Obtém nome do usuário atual do token
    const senderName = decodedToken?.name || decodedToken?.username || 'Usuário';
    ...
    const notificationBody = {
      ...
      title: 'Nova Mensagem',
      content: `Nova mensagem de ${senderName}: ${preview}`,
      type: 'CHAT_MESSAGE'
    };

    const response = await fetch(`${BASE_URL}/v1/notification/send`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': token
